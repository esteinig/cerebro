            ...
            match subcommand {
                StackCommands::Deploy(args) => {
                    let mut stack = match (&args.config, &args.config_file) {
                        ...
                        (_, Some(path)) => cerebro::stack::deploy::StackConfig::from_toml(path)?,
                    ...
                    // Emit the deployment's retention policy for the
                    // server / fs / tower services to load via env_file.
                    let retention_env =
                        cerebro::stack::deploy::write_retention_env(&args.outdir, args)?;
                    log::info!(
                        "Wrote retention policy to {} (load via env_file)",
                        retention_env.display()
                    );
