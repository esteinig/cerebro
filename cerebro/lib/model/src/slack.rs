...
impl MessageGenerator {
    ...
    pub fn watcher_setup(&self, watcher_name: &str, watcher_loc: &str) -> SlackMessage {
        SlackMessage::new(
            &self.channel,
            &format!("[{watcher_name}@{watcher_loc}] New watcher has been initialised"),
        )
    }
    pub fn input_detected(
        &self,
        run_id: &str,
        watcher_name: &str,
        watcher_loc: &str,
        ...
        SlackMessage::new(
            &self.channel,
            &format!("[{watcher_name}@{watcher_loc}::{run_id}] New run input detected"),
        )
    }
...
impl SlackMessage {
    pub fn new(channel: &str, message: &str) -> Self {
        Self {
            ...
            blocks: vec![SlackMessageSectionBlock::new(TextObject::markdown(message))],
        }
    }
...
impl SlackBlockingClient {
    pub fn new(token: &str) -> Self {
        let client = reqwest::blocking::Client::builder()
            .build()
            .expect("Failed to initialize Reqwest blocking client!");
        Self {
            url: String::from("https://slack.com/api/chat.postMessage"),
    ...
    pub fn send(&self, message: &SlackMessage) -> Result<(), SlackError> {
        let response: reqwest::blocking::Response = self
            ...
            .json(&message)
            .send()
            .expect("Failed to send POST request to Slack API");

        let response_data: SlackMessengerResponse = response
            .json()
            .expect("Failed to read JSON response from Slack API");
