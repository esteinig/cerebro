...
use cerebro_model::api::cerebro::schema::{
    PathogenDetectionTableSchema, QualityControlTableSchema,
};
...
use cerebro_client::terminal::{
    App, Commands, DatabaseCommands, JobsCommands, ProjectCommands, StageCommands, TeamCommands,
    TowerCommands, TrainingCommands, WatcherCommands,
};
...
fn main() -> anyhow::Result<()> {
    ...
    let client = CerebroClient::new(
        ...
        cli.token,
        match &cli.command {
            Commands::Login(_) | Commands::PingStatus(_) => true,
            _ => false,
        },
    ...
    match &cli.command {
        ...
        Commands::Login(args) => client.login_user(&args.email, args.password.clone(), args.bot)?,
        // Ping servers with token
        Commands::PingServer(_) => client.ping_servers()?,
        // Ping server without token
        Commands::PingStatus(_) => client.ping_status()?,
        ...
        Commands::UploadPathogen(args) => {
            ...
            if quality.id != pathogen.id {
                return Err(HttpClientError::PathogenIdentifiersNotMatched(
                    quality.id,
                    pathogen.id,
                )
                .into());
            ...
            let (team, database, project, run_id) = match &args.stage_json {
                ...
                None => {
                    match (
                        ...
                        (Some(team), Some(database), Some(project), Some(run_id)) => {
                            (team, database, project, run_id)
                        }
            ...
            let cerebro = Cerebro::from(
                &quality.id,
                &quality,
                &pathogen.get_taxa(&args.taxonomy, args.strict, args.gtdb_break_monophyly)?,
            ...
            client.upload_models(&Vec::from([cerebro]))?;
        }
        // Process and upload sample model to database
        ...
        Commands::UploadPanviral(args) => {
            ...
            if quality.id != panviral.id {
                return Err(HttpClientError::PanviralIdentifiersNotMatched(
                    quality.id,
                    panviral.id,
                )
                .into());
            ...
            let cerebro = Cerebro::from(
                &quality.id,
                &quality,
                &panviral.get_taxa(&args.taxonomy, args.strict, args.gtdb_break_monophyly)?,
                args.sample_sheet.clone(),
                args.pipeline_config.clone(),
                Some(run_id),
                args.run_date.clone(),
        ...
        Commands::DownloadModels(args) => {
            ...
            client.download_models(&args.outdir, args.summary_table.clone())?;
        }

        Commands::UpdateModels(args) => {
            client.update_models(args.run_tsv.clone())?;
        }
        Commands::DeleteModels(args) => {
            client.delete_models(args.name_tsv.clone())?;
        }
        Commands::Tower(subcommand) => match subcommand {
            TowerCommands::Register(args) => {
                let schema =
                    RegisterTowerSchema::new(&args.name, &args.location, args.pipelines.clone());

                client.register_tower(&schema, true)?;
            ...
            TowerCommands::Delete(args) => {
                if args.all {
                    ...
                    if confirmation {
                        client.delete_tower(None, None, None, None)?;
                    }
                } else {
                    client.delete_tower(
                        args.id.clone(),
        ...
        Commands::Watcher(subcommand) => match subcommand {
            WatcherCommands::Register(args) => {
                ...
                client.register_watcher(&register_watcher_schema, true)?;
            ...
            WatcherCommands::Ping(args) => {
                let watcher_id = match (args.json.clone(), args.id.clone()) {
                    ...
                    (None, Some(id)) => id.to_owned(),
                    (None, None) => {
                        return Err(HttpClientError::WatcherIdentifierArgNotFound.into())
                    }
                };
            ...
            WatcherCommands::Delete(args) => {
                if args.all {
                    ...
                    if confirmation {
                        client.delete_watcher(None, None, None, None)?;
                    }
                } else {
        ...
        Commands::Stage(subcommand) => match subcommand {
            ...
            StageCommands::Delete(args) => {
                ...
                if args.all {
                    let confirmation = dialoguer::Confirm::new()
                        .with_prompt(
                            "Do you want to delete ALL staged samples for this staging area?",
                        )
                        .interact()
                        .unwrap();

                    if confirmation {
                        client.delete_staged_sample(&tower_id, None, None, None)?;
                    }
                } else {
        ...
        Commands::GetNegativeControls(args) => {
            let data = client.get_negative_controls(&args.sample_id.clone())?;
        ...
        Commands::GetQualityControlTable(args) => {
            let schema =
                QualityControlTableSchema::new(args.sample_ids.clone().unwrap_or_default());

            let sample_summaries = client.get_quality_control(&schema, args.output.clone())?;
        ...
        Commands::GetPathogenDetectionTable(args) => {
            ...
            client.get_pathogen_detection(&schema, args.output.clone())?;
        }
        ...
        Commands::Training(subcommand) => {
            match subcommand {
                ...
                TrainingCommands::Upload(args) => {
                    client.upload_training_prefetch(
                        &args.prefetch,
                        &args.collection,
                        &args.description,
                        args.preselect.clone(),
                    )?;
                }
                ...
                TrainingCommands::DeleteDataset(args) => {
                    if args.name.is_none() {
                        let confirmation = dialoguer::Confirm::new()
                            .with_prompt(
                                "Do you want to delete ALL entries for this training collection?",
                            )
                            .interact()
                            .unwrap();
                        ...
                        if confirmation {
                            client
                                .delete_training_data(args.collection.clone(), args.name.clone())?;
                        }
                    } else {
                        client.delete_training_data(args.collection.clone(), args.name.clone())?;
                    }
                }

                // List session data in a team training database
                TrainingCommands::ListSessions(args) => {
                    client.list_training_sessions(
                        args.completed,
        ...
        Commands::Team(subcommand) => {
            match subcommand {
                ...
                TeamCommands::Create(args) => {
                    client.create_team(&args.name, &args.description)?;
                }
            }
        }
        Commands::Database(subcommand) => {
            match subcommand {
                ...
                DatabaseCommands::Create(args) => {
                    client.create_database(&args.name, &args.description)?;
                }
        ...
        Commands::Project(subcommand) => {
            match subcommand {
                ...
                ProjectCommands::Create(args) => {
                    client.create_project(&args.name, &args.description)?;
                }
                ...
                JobsCommands::ScheduleJob {
                    ...
                    let run_at_dt = chrono::DateTime::parse_from_rfc3339(run_at)
                        .map_err(|e| {
                            HttpClientError::DataResponseFailure(
                                reqwest::StatusCode::BAD_REQUEST,
                                format!("Invalid --run-at RFC3339: {e}"),
                            )
                        })?
