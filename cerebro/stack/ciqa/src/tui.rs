...
impl SinSignal {
    const fn new(interval: f64, period: f64, amplitude: f64, phase: f64) -> Self {
        Self {
            x: 0.0,
            interval,
            period,
...
impl Iterator for SinSignal {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let y =
            ((self.x / self.period) * std::f64::consts::TAU + self.phase).sin() * self.amplitude;
        let p = (self.x, y);
...
fn make_themes() -> Vec<PastelTheme> {
    let base = PastelTheme {
        header_models: Style::default()
            .fg(hex_rgb(0xFF66CC))
            .add_modifier(Modifier::BOLD),
        _header_metrics: Style::default()
            .fg(hex_rgb(0x66FFCC))
            .add_modifier(Modifier::BOLD),
        header_stats: Style::default()
            .fg(hex_rgb(0xC4A1FF))
            .add_modifier(Modifier::BOLD),
        header_detail: Style::default()
            .fg(hex_rgb(0xFFBF80))
            .add_modifier(Modifier::BOLD),
        _footer: Style::default()
            .fg(hex_rgb(0x80859A))
        ...
        matrix_consensus: Style::default()
            .fg(hex_rgb(0xFFFFFF))
            .add_modifier(Modifier::BOLD),
        matrix_header: Style::default()
            .fg(hex_rgb(0x8890A0))
            .add_modifier(Modifier::BOLD),
    };
    let alt = PastelTheme {
...
impl App {
    ...
    fn next_model(&mut self) {
        let i = match self.selected.selected() {
            Some(i) if i + 1 < self.models.len() => i + 1,
            _ => 0,
        };
    ...
    fn theme(&self) -> &PastelTheme {
        &self.themes[self.theme_index]
    }
    fn on_tick(&mut self) {
        for _ in 0..5 {
            if let Some(p) = self.strand_a.next() {
                self.dna_a.push(p);
            }
            if let Some(p) = self.strand_b.next() {
                self.dna_b.push(p);
            }
        }
...
fn draw_statistics(f: &mut ratatui::Frame, area: Rect, app: &App, theme: &PastelTheme) {
    ...
    f.render_widget(
        Block::default()
            .borders(Borders::ALL)
            .title(Span::styled("Reference Plate", theme.header_stats)),
        area,
    );
}
...
fn _draw_line_demo(f: &mut ratatui::Frame, area: Rect) {
    ...
    let chart = Chart::new(vec![ds])
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(Line::from("Line").cyan().bold().centered()),
        )
        .x_axis(
            Axis::default()
                .bounds([0.0, 5.0])
...
fn _draw_bar_demo(f: &mut ratatui::Frame, area: Rect) {
    ...
    let chart = Chart::new(vec![ds])
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(Line::from("Bar").cyan().bold().centered()),
        )
        .x_axis(
            Axis::default()
                .bounds([0.0, 100.0])
...
fn _draw_scatter_demo(f: &mut ratatui::Frame, area: Rect) {
    let ds1 = Dataset::default()
        ...
        .data(&[(1., 3.), (2., 4.), (3., 5.), (4., 4.)]);
    let ds2 = Dataset::default()
        .name("M")
        .graph_type(GraphType::Scatter)
        .marker(symbols::Marker::Braille)
        .style(Style::default().fg(Color::Magenta))
        .data(&[(1., 1.), (2., 1.5), (3., 2.), (4., 1.8)]);
    let ds3 = Dataset::default()
        .name("H")
        .graph_type(GraphType::Scatter)
        .marker(symbols::Marker::Block)
        .style(Style::default().fg(Color::Yellow))
        .data(&[(1., 2.), (2., 2.2), (3., 2.6), (4., 3.0)]);
    let chart = Chart::new(vec![ds1, ds2, ds3])
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(Line::from("Scatter").on_cyan().bold().centered()),
        )
        .x_axis(
            Axis::default()
                .bounds([0.0, 5.0])
...
fn build_performance_barchart(app: &App, theme: &PastelTheme) -> BarChart<'static> {
    ...
    let groups: Vec<BarGroup<'static>> = perfs
        ...
        .map(|p| {
            ...
            BarGroup::default().label(Line::from(short)).bars(&bars)
        })
        .collect();
...
fn draw_confusion_matrix(f: &mut ratatui::Frame, area: Rect, model: &Model, theme: &PastelTheme) {
    ...
    let fp = model.fp;
    let tn = model.tn;
    let _sens = model.sensitivity();
    let _spec = model.specificity();
...
fn draw_detail(f: &mut ratatui::Frame, area: Rect, app: &App, theme: &PastelTheme) {
    ...
    f.render_widget(
        Block::default()
            .borders(Borders::ALL)
            .title(Span::styled("Detail", theme.header_detail)),
        area,
    );
    ...
    let text = Text::from(vec![
        ...
        Line::from(""),
        Line::from(format!(
            "GPU {gmin}/{gmed:.1}/{gmax}  RUN {rmin}/{rmed:.1}/{rmax}"
        )),
        Line::from(""),
