...
fn main() -> anyhow::Result<(), anyhow::Error> {
    ...
    match cli.command {
        ...
        Commands::PlotRadar(args) => {
            ...
            let mut series: Vec<(String, [f64; 5])> = Vec::new();
            for (i, path) in args.input.iter().enumerate() {
                ...
                series.push((
                    if !args.labels.is_empty() {
                        ...
                        get_file_component(path, FileComponent::FileStem)?
                    },
                    [
                        data.consensus.npv / 100.0,
                        data.consensus.specificity / 100.0,
                        data.consensus.ppv / 100.0,
                        replicate_cert,
                        data.consensus.ppv / 100.0,
                    ],
                ));
        ...
        Commands::PlotReview( args ) => {
            ...
            // Log mean, SD, range and 95% CI per category for the two metrics
            // selected by --mode (sensitivity/specificity or ppv/npv).
            log_review_summary_stats(&data, args.mode, &args.stats)?;
        ...
        Commands::PlotReviewMatrix( args ) => {

            // Assemble per-outcome color overrides; unset outcomes fall back to
            // the built-in diagnostic-review palette.
            let colors = ReviewMatrixColors {
                tp: args.color_tp,
                tn: args.color_tn,
                fp: args.color_fp,
                fn_: args.color_fn,
                na: args.color_na,
                indeterminate: args.color_indeterminate,
                control: args.color_control,
                unknown: args.color_unknown,
            };

            let palette = Palette::diagnostic_review_custom(&colors)?;

            let shape = if args.square {
                CellShape::Square { border_width: args.square_border }
            } else {
                CellShape::Circle
            };

            plot_review_matrix_from_tsv(
                &args.matrix,
                &args.output,
                &palette,
                shape,
        ...
        Commands::TaxonTable( args ) => {

            write_taxon_evidence_table(&args.prefetch, &args.output)?;

            log::info!("Wrote taxon evidence table to: {}", args.output.display());

        },
        Commands::RedactDiagnostic( args ) => {

            let redaction = if args.null {
                serde_json::Value::Null
            } else {
                serde_json::Value::String(args.placeholder.clone())
            };
            ...
            for input in &args.json {
                let output = if args.in_place {
                    ...
                    let name = input.file_name()
                        .ok_or_else(|| CiqaError::InvalidInputPath(input.clone()))?;
                    args.outdir.join(name)
                };

                let n = redact_diagnostic_json(input, &output, &redaction)?;

                log::info!(
                    "Redacted {} clinical/note field(s): {} -> {}",
                    n, input.display(), output.display()
                );
            }
