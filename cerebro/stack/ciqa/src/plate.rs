use crate::{
    error::CiqaError,
    terminal::ReviewArgs,
    utils::{get_file_component, read_tsv, write_tsv, FileComponent},
};
...
pub fn parse_dir_components(dir_name: &str) -> Option<(String, String, String, u32)> {
    ...
    // 4=replicate
    let re =
        Regex::new(r"^qwen3-(\d+b)-(q\d+)(?:[-_][0-9a-z]+)*_(clinical|noclinical)(?:_.+)?_(\d+)$")
            .ok()?;
...
impl FromSampleType for SampleContext {
    fn from_sample_type(sample_type: SampleType) -> SampleContext {
        match sample_type {
            ...
            _ => unimplemented!(
                "Variant of sample type not implemented for FromSampleType -> Clinical Context"
            ),
        }
    }
}
...
impl SampleReview {
    pub fn from_gpt(path: &Path) -> Result<Self, CiqaError> {
        ...
        let sample_id = stem_parts
            .next()
            .ok_or(CiqaError::SampleIdentifierNotFound)?;

        let test_result = if diagnostic_result.diagnosis == Diagnosis::Infectious
            || diagnostic_result.diagnosis == Diagnosis::InfectiousReview
        {
            Some(TestResult::Positive)
        } else if diagnostic_result.diagnosis == Diagnosis::NonInfectious
            || diagnostic_result.diagnosis == Diagnosis::NonInfectiousReview
        {
            Some(TestResult::Negative)
        } else {
            log::warn!(
                "Diagnosis from MetaGPT is: {:?} ({})",
                diagnostic_result,
                sample_id
            );
...
impl DiagnosticSummary {
    pub fn from_stats(data: Vec<DiagnosticStats>) -> Self {
        ...
        let filtered: Vec<DiagnosticStats> =
            data.into_iter().filter(|s| s.name != "consensus").collect();
...
pub fn average_replicate_certainty(
    ...
    for row in 0..nrows {
        for col in data {
            let rev = &col[row];
            match rev.outcome {
                DiagnosticOutcome::TruePositive | DiagnosticOutcome::TrueNegative => {
                    total_good += 1;
                    total_count += 1;
                }
...
impl DiagnosticData {
    ...
    pub fn get_reference_from_json<P: AsRef<Path>>(
        path: P,
    ) -> Result<Option<Vec<DiagnosticReview>>, CiqaError> {
        let reference_data = Self::from_json(path)?;

        let reference_review: Vec<Vec<DiagnosticReview>> = reference_data
            .stats
            .iter()
            .filter_map(|d| {
                if d.name == String::from("consensus") {
                    None
                } else {
                    Some(d.data.clone())
                }
    ...
    pub fn plot_summary(
        ...
        let data_columns: Vec<Vec<DiagnosticReview>> = self
            .stats
            .iter()
            .filter_map(|d| {
                if d.name == String::from("consensus") {
        ...
        let consensus_column = self.stats.iter().find_map(|d| {
            if d.name == String::from("consensus") {
                Some(d.data.clone())
            } else {
                None
...
pub trait RgbColor {
    fn from_hex_str(hex: &str) -> Result<Self, String>
    where
        Self: Sized;
}
...
impl RgbColor for RGBColor {
        ...
        Self: Sized,
    {
        ...
        if hex.len() != 6 {
            return Err(format!(
                "Invalid hex code length for '{}'. Expected 6 characters.",
                hex
            ));
        }
...
pub fn get_diagnostic_stats(
    args: &ReviewArgs,
    reference_plate: &mut ReferencePlate,
    review_name: &str,
) -> Result<DiagnosticStats, CiqaError> {
    ...
    for dr in &diagnostic_review {
        if dr.outcome == DiagnosticOutcome::FalseNegative
            || dr.outcome == DiagnosticOutcome::FalsePositive
        {
            log::info!(
                "{} => {:<14}{}",
                dr.sample_id,
    ...
    let stats = reference_plate.compute_statistics(&review_name, diagnostic_review);
...
impl ReferencePlate {
    ...
    pub fn from_review(
        ...
        missing_orthogonal: MissingOrthogonal,
        diagnostic_agent: bool,
    ) -> Result<Self, CiqaError> {
    ...
    pub fn from_tsv(
        path: &Path,
        missing_orthogonal: MissingOrthogonal,
        ignore_taxstr: Option<String>,
    ) -> Result<Self, CiqaError> {
        let rows = read_tsv::<ReferencePlateTsvRow>(path, false, true)?;

        let reference: Vec<_> = rows
            .into_iter()
            .map(|row| SampleReference::from_tsv(row, ignore_taxstr.clone()))
            .collect();
    ...
    pub fn get_samples(reference: &Vec<SampleReference>) -> Vec<String> {
        reference
            .into_iter()
            .filter(|r| {
                !([SampleType::Ntc, SampleType::Env, SampleType::Pos].contains(&r.sample_type))
            })
            .map(|r| r.sample_id.clone())
            .collect()
    }
    ...
    pub fn compute_diagnostic_review(&self) -> Result<Vec<DiagnosticReview>, CiqaError> {
        if let Some(ref reviews) = self.review {
            ...
            let review_map: HashMap<&str, &SampleReview> =
                reviews.iter().map(|r| (r.sample_id.as_str(), r)).collect();
            ...
            Ok(self
                ...
                .map(|reference| {
                    let (outcome, review) =
                        if let Some(review) = review_map.get(reference.sample_id.as_str()) {
                            (
                                compare_sample_review(
                                    reference,
                                    review,
                                    self.missing_orthogonal.clone(),
                                ),
                                Some(*review),
                            )
                        } else {
                            if [SampleType::Ntc, SampleType::Env, SampleType::Pos]
                                .contains(&reference.sample_type)
                            {
            ...
            log::warn!(
                "Review data was not provided to `ReferencePlate` for diagnostic evaluation"
            );
            Ok(Vec::new())
        }
    }
    ...
    pub fn compute_statistics(
        &self,
        name: &str,
        diagnostic_review: Vec<DiagnosticReview>,
    ) -> DiagnosticStats {
        // Filter out DiagnosticOutcome::Indeterminate
        let filtered: Vec<DiagnosticReview> = diagnostic_review
            .iter()
            .filter(|d| {
                !([
                    DiagnosticOutcome::Indeterminate,
        ...
        let tp = filtered
            .iter()
            .filter(|d| matches!(d.outcome, DiagnosticOutcome::TruePositive))
            .count();
        let fp = filtered
            .iter()
            .filter(|d| matches!(d.outcome, DiagnosticOutcome::FalsePositive))
            .count();
        let tn = filtered
            .iter()
            .filter(|d| matches!(d.outcome, DiagnosticOutcome::TrueNegative))
            .count();
        let fn_ = filtered
            .iter()
            .filter(|d| matches!(d.outcome, DiagnosticOutcome::FalseNegative))
            .count();
    ...
    pub fn prevalence_contamination(
        ...
        log::info!(
            ...
            client.db.clone().unwrap_or_default(),
            client.project.clone().unwrap_or_default()
        );

        let dna_taxids =
            client.get_prevalence_contamination(contam_config, vec![String::from("DNA")])?;
        let rna_taxids =
            client.get_prevalence_contamination(contam_config, vec![String::from("RNA")])?;
    ...
    pub fn prefetch(
        ...
        log::info!(
            "Fetching primary threshold data for sample '{}'",
            config.sample
        );
        ...
        log::info!(
            "Fetching secondary threshold data for sample '{}'",
            config.sample
        );
        ...
        log::info!(
            "Fetching target threshold data for sample '{}'",
            config.sample
        );
...
fn compare_sample_review(
    ...
    if reference
        .note
        .clone()
        .is_some_and(|x| x.contains("no_data"))
    {
...
pub fn plot_stripplot<B: DrawingBackend>(
    ...
    for file in file_order {
        file_labels.push(get_file_component(&file, FileComponent::FileStem)?)
    }
        ...
        for (idx, cat_name) in categories.iter().enumerate() {
            if let Some(diag_data) = data.get(cat_name) {
                ...
                if !vals1.is_empty() {
                    ...
                    chart
                        .draw_series(std::iter::once(PathElement::new(
                            vec![(lw, center_y1), (q1, center_y1)],
                            whisker_style.clone(),
                        )))
                        .map_err(|e| CiqaError::StripPlotError(e.to_string()))?;
                    chart
                        .draw_series(std::iter::once(PathElement::new(
                            vec![(q3, center_y1), (uw, center_y1)],
                            whisker_style.clone(),
                        )))
                        .map_err(|e| CiqaError::StripPlotError(e.to_string()))?;
                }
                // Measure 2 values
                let mut vals2: Vec<f64> = stats
                    .iter()
                    .map(|s| match mode {
                ...
                if !vals2.is_empty() {
                    ...
                    chart
                        .draw_series(std::iter::once(PathElement::new(
                            vec![(lw, center_y2), (q1, center_y2)],
                            whisker_style.clone(),
                        )))
                        .map_err(|e| CiqaError::StripPlotError(e.to_string()))?;
                    chart
                        .draw_series(std::iter::once(PathElement::new(
                            vec![(q3, center_y2), (uw, center_y2)],
                            whisker_style.clone(),
                        )))
                        .map_err(|e| CiqaError::StripPlotError(e.to_string()))?;
                }
        ...
        for (idx, cat_name) in categories.iter().enumerate() {
            if let Some(diag_data) = data.get(cat_name) {
                ...
                let (ci_m1, ci_m2) = match mode {
                    StatsMode::SensSpec => (
                        summary.ci_sensitivity.clone(),
                        summary.ci_specificity.clone(),
                    ),
...
pub fn plot_diagnostic_matrix(
    ...
    let panel_column_headers = column_header.panel_column_header(panels_x, panels_y);
    ...
    let consensus_idx: Option<usize> = if consensus.is_some() {
        Some(data.len())
    } else {
        None
    };
    let extra_gap_per_panel = if consensus_idx.is_some() {
        consensus_gap_px
    } else {
        0.0
    };

    let height_px = ((panels_y as f64 * chunk_size as f64) * (cell_px as f64 + row_padding)
    ...
    let width_px = ((panels_x as f64 * total_cols as f64) * stride
    ...
    let get_style = |rev: &DiagnosticReview| palette.colors[rev.outcome.index()].filled();
    ...
    // Draw each panel
    for (panel, (chunk_idx, chunk)) in panels
        .iter()
        .zip(sample_labels.chunks(chunk_size).enumerate())
    {
            ...
            for row in 0..nrows_chunk {
                ...
                match shape {
                    CellShape::Circle => {
                        ...
                        panel_area.draw(&Circle::new(
                            (cx, cy),
                            (cell_px as f64 / 2.0) as i32,
                            style.clone(),
                        ))?;
                    }
        ...
        for (row, label) in chunk.iter().enumerate() {
            let y0 =
                row as f64 * (cell_px as f64 + row_padding) + (cell_px as f64 / 2.0) + row_padding;
        ...
        if panel_column_headers[chunk_idx] {
            for col_idx in 0..columns.len() {
                // only real columns get rotated headers
                let x0 = x_for(col_idx);
    ...
    let text_w = |s: &str, bolding: bool| -> i32 {
        ...
        .into_text_style(&root);
        root.estimate_text_size(s, &ts)
            .unwrap_or((((font_size as f64) * 0.60).round() as u32, 0.0 as u32))
            .0 as i32
    };
...
impl PanelColumnHeader {
    pub fn panel_paddings_top(
        ...
        let total_padding_y = match self {
            ...
            PanelColumnHeader::FirstRow => {
                (base_padding + header_padding) + (base_padding * (num_panels_y - 1))
            }
        };
    ...
    pub fn panel_column_header(&self, num_panels_x: usize, num_panels_y: usize) -> Vec<bool> {
        ...
        match self {
            ...
            PanelColumnHeader::FirstRow => [
                vec![true; num_panels_x],
                vec![false; num_panels - num_panels_x],
            ]
            .concat(),
        }
    }
...
pub fn plot_qc_summary_matrix(
    ...
    let mut categories = vec!["Input Reads", "Output Reads", "ERCC | EDCC", "Read Quality"];
    ...
    let width_px = ((panels_x as f64 * total_cols as f64) * (cell_px as f64 + col_padding)
        + (panels_x as f64 * (panel_padding_left + panel_padding_right))
        + (2.0 * outer_margin))
        .ceil() as u32;
    ...
    let root = SVGBackend::new(output, (width_px, height_px as u32)).into_drawing_area();
