...
use crate::{
    error::WorkflowError,
    nextflow::{panviral::PanviralOutput, pathogen::PathogenDetectionOutput},
    taxa::taxon::{collapse_taxa, Taxon, TaxonExtraction},
    utils::read_tsv,
};
...
impl Alignment {
    pub fn from_panviral(output: &PanviralOutput) -> Result<Self, WorkflowError> {
        Ok(Self {
            id: output.id.clone(),
            records: output.vircov.records.clone(),
        })
    }
    pub fn from_pathogen(output: &PathogenDetectionOutput) -> Result<Self, WorkflowError> {
        Ok(Self {
            id: output.id.clone(),
            records: output
                .profile
    ...
    fn get_taxa(
        ...
        for record in &self.records {
            let taxid = match &record.taxid {
                None => {
                    log::error!(
                        "Taxid not found for record with reference sequence: {}",
                        record.reference
                    );
