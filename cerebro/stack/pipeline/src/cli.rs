use cerebro_pipeline::{
    modules::{
        alignment::Alignment,
        pathogen::{write_pathogen_table, PathogenDetection, PathogenDetectionFilter},
        quality::{write_quality_table, QualityControl},
    },
    nextflow::{
        mag::BlastTaxidMethod,
        panviral::PanviralOutput,
        pathogen::PathogenDetectionOutput,
        quality::{QualityControlFiles, QualityControlOutput},
    },
    terminal::{App, Commands, ProcessCommands, TableCommands, ToolsCommands},
    tools::{scan::ScanReads, sheet::SampleSheet, umi::Umi},
    utils::init_logger,
};
use clap::Parser;
...
fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Commands::Process(subcommand) => match subcommand {
            ...
            ProcessCommands::Pathogen(args) => {
                ...
                let pathogen_detection =
                    PathogenDetection::from_pathogen(&output, &quality_control, args.paired_end)?;
            ...
            ProcessCommands::Quality(args) => {
                let output =
                    QualityControlOutput::from(&args.input, args.id.clone(), args.qc_fail_ok)?;
        ...
        Commands::Tools(subcommand) => match subcommand {
        ...
        Commands::Table(subcommand) => match subcommand {
            TableCommands::QualityControl(args) => {
                write_quality_table(&args.json, &args.reads, &args.controls, &args.background)?
            }

            TableCommands::PathogenDetection(args) => write_pathogen_table(
                &args.json,
                &args.output,
                args.taxonomy.clone(),
                args.rank.clone(),
