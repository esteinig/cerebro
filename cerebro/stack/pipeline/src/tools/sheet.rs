...
impl SampleSheet {
    pub fn new(
        ...
        for directory in directories {
            ...
            let entries_directory = sample_files
                .iter()
                .map(
                    |(sample_id, file_paths)| -> Result<SampleSheetEntry, WorkflowError> {
                        let run = match &run_id {
                            Some(id) => id.to_string(),
                            None => {
                                crate::utils::get_file_component(directory, FileComponent::FileStem)
                                    .map_err(|_| {
                                        WorkflowError::SampleSheetEntryRunDirectoryPath(format!(
                                            "Failed to extract path as string: {:?}",
                                            &directory
                                        ))
                                    })?
                            }
    ...
    pub fn from(input: &PathBuf) -> Result<Self, WorkflowError> {
        let mut reader = csv::ReaderBuilder::new()
            .from_path(input)
            .map_err(|_| WorkflowError::SampleSheetCsvReader(format!("{:?}", input)))?;
        let mut entries = Vec::new();
    ...
    pub fn write(&self, output: &PathBuf) -> Result<(), WorkflowError> {
        let mut writer = Writer::from_path(output)
            .map_err(|_| WorkflowError::SampleSheetCsvWriter(format!("{:?}", output)))?;
    ...
    pub fn get_run_id(&self, sample_id: &str) -> Option<String> {
        let matches: Vec<&SampleSheetEntry> = self
            .entries
            .iter()
...
fn check_unique_sample_id(entries: &Vec<SampleSheetEntry>) -> Result<(), WorkflowError> {
    ...
    let duplicates: Vec<&str> = id_count
        .iter()
...
pub fn get_files(
    ...
    symlinks: bool,
    recursive: bool,
    prefix_subset: &Vec<String>,
) -> Result<(HashMap<String, Vec<PathBuf>>, Vec<String>), WorkflowError> {
    let glob =
        Glob::new(paired_glob).map_err(|_| WorkflowError::GlobCreate(paired_glob.to_string()))?;

    let mut walk_behaviour = wax::WalkBehavior::default();

    walk_behaviour.link = if symlinks {
        wax::LinkBehavior::ReadTarget
    } else {
        wax::LinkBehavior::ReadFile
    };
    ...
    for entry in glob.walk_with_behavior(directory, walk_behaviour) {
        ...
        let sample_id = entry
            .matched()
            .get(1)
            .ok_or_else(|| WorkflowError::GlobMatchSampleIdentifier(format!("{:?}", file_path)))?;
