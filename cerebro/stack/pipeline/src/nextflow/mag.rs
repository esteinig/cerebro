...
use crate::utils::{get_file_by_name, get_file_component, read_tsv, FileComponent};
...
impl MetagenomeAssemblyOutput {
    pub fn from(
        ...
        Ok(Self {
            ...
            blast: match files.blast {
                Some(ref path) => {
                    Some(Self::parse_blast_records(&id, path, taxonomy, blast_taxid)?)
                }
                None => None,
            },
        })
    ...
    pub fn parse_blast_records(
        id: &str,
        path: &PathBuf,
        taxonomy: Option<&GeneralTaxonomy>,
        blast_taxid: BlastTaxidMethod,
    ) -> Result<Vec<ContigBlastRecord>, WorkflowError> {
        ...
        for (_, records) in grouped_records {
            let contig_record =
                ContigBlastRecord::from_blast_records(id, &records, taxonomy, blast_taxid.clone())?;
...
impl ContigBlastRecord {
    pub fn from_blast_records(
        id: &str,
        records: &Vec<BlastRecord>,
        taxonomy: Option<&GeneralTaxonomy>,
        method: BlastTaxidMethod,
    ) -> Result<Self, WorkflowError> {
        ...
        let read_coverage = match record_name_components.contains(&"cov") {
            true => match record_name_components.last() {
                Some(cov) => cov
                    .parse::<f64>()
                    .map_err(WorkflowError::BlastLcaFloatFieldConversion)?,
        ...
        let (record, taxid, taxrank) = match method {
            ...
            BlastTaxidMethod::LCA => {
                ...
                let taxids_unique: Vec<String> = records
                    .iter()
                    .map(|x| x.taxid.to_string())
                    .unique()
                    .collect();
                ...
                {
                    Some(record) => {
                        let taxid = if no_lca_taxid {
                            record.taxid.to_owned()
                        } else {
                            lca_taxid
                        };
...
pub fn get_lca_taxid(
    taxids_unique: Vec<String>,
    ...
    match taxids_unique.len() {
        ...
        _ => {
            ...
            let lineage_sets: Vec<HashSet<&str>> = lineages
                .clone()
                .into_iter()
                .map(|x| HashSet::from_iter(x))
                .collect();
            // Take the set intersection of lineages
            let common_lineages: HashSet<&str> =
                if let Some((first, rest)) = lineage_sets.split_first() {
