...
use cerebro_model::api::auth::schema::{
    AuthLoginSchema, AuthOneTimeCheckSchema, AuthResetPasswordSchema, TokenDetails,
};
...
fn one_time_token_rate_limiter_factory(
...
async fn login_handler(
        ...
        .unwrap()
    {
        ...
        None => {
            ...
            match log_user_login(
                &data,
                &body.email,
                Action::InvalidEmail,
                "User login attempt failed due to invalid email",
                &req,
            )
            .await
            {
    ...
    if !password_is_valid {
        ...
        match log_user_login(
            ...
            "User login attempt failed due to invalid password",
            &req,
        )
        .await
        {
    ...
    if !user.verified {
        ...
        match log_user_login(
            &data,
            &body.email,
            Action::InvalidVerification,
            "User login attempt failed due to invalid verification",
            &req,
        )
        .await
        {
    ...
    match log_user_login(
        &data,
        &body.email,
        Action::ValidCredentials,
        "User login attempt was successful",
        &req,
    )
    .await
    {
    ...
    let (access_token_max_age, refresh_token_max_age) = match query.role {
                ...
                .await
                {
                    ...
                    Err(_) => {
                        return HttpResponse::InternalServerError().json(
                            serde_json::json!({"status": "fail", "message": "Not authorized"}),
                        )
                    }
                };
                (
                    data.env.security.token.expiration.access_max_age_bot,
                    data.env.security.token.expiration.refresh_max_age_bot,
                )
            } else {
                match log_user_login(
                    &data,
                    &body.email,
                    Action::InvalidRole,
                    "Bot authorization unsuccessful",
                    &req,
                )
                .await
                {
        ...
        _ => (
            ...
            data.env.security.token.expiration.refresh_max_age,
        ),
    };
    ...
    if let Err(_) = access_result {
        return HttpResponse::UnprocessableEntity()
            .json(serde_json::json!({"status": "error", "message": "Not authorized"}));
        // change to opaque message
    }
...
async fn refresh_access_token_handler(
    ...
    let access_cookie = Cookie::build("access_token", access_token_details.token.clone().unwrap())
        ...
        .max_age(ActixWebDuration::new(
            data.env.security.token.expiration.access_max_age * 60,
            0,
        ))
        .domain(&data.env.security.cookies.domain)
        .http_only(true)
...
async fn verification_email_handler(
    ...
    // Find the user in the database
    let user = match user_collection.find_one(doc! { "id": &user_id }).await {
        Ok(Some(user)) => user,
        Ok(None) => {
            return HttpResponse::BadRequest()
                .json(serde_json::json!({"status": "fail", "message": "Could not find user"}))
        }
        Err(_) => {
            return HttpResponse::InternalServerError()
                .json(serde_json::json!({"status": "fail", "message": "Failed to find user"}))
        }
    };
...
#[post(
    ...
    wrap = "one_time_token_rate_limiter_factory()"
)]
...
async fn reset_password_email_handler(
    ...
    let (token_type, access_token_details, user) =
        match check_one_time_token_and_user(&data, &user_collection, &body.access_token).await {
...
async fn reset_password_handler(
    ...
    match token_type {
        OneTimeTokenType::PasswordReset => {
            ...
            let hashed_password =
                match Argon2::default().hash_password(&body.password.as_bytes(), &salt) {
...
async fn create_one_time_token(
    ...
    ttl: &i64,
) -> Result<TokenDetails, HttpResponse> {
    let error_response: HttpResponse = HttpResponse::Forbidden().json(
        serde_json::json!({"status": "error", "message": "Token is invalid or session has expired"}),
    );
