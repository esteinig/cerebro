...
use cerebro_model::api::users::model::{Role, User};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            serde_json::to_string(&self).unwrap_or(String::from(
                "Failed error response serialization for display"
            ))
        )
    }
}
                ...
                team: match query_map.get("team") {
                    ...
                    None => {
                        return Err(ErrorResponse {
                            ...
                            message: String::from(
                                "Failed to provide team query parameter for data access",
                            ),
                        })
                    }
                },
        ...
        let access_token = match access_token {
            ...
            None => {
                let json_error = ErrorRefreshResponse {
                    // this may happen if client-side fetch is called after cookie expiration (and no server-side refresh initiated)
        ...
        let user_id_redis_result = async move {
            ...
            match redis_result {
                ...
                Err(_) => Err(ErrorUnauthorized(ErrorRefreshResponse {
                    // user session may be expired (max_age)
                    status: "fail".to_string(),
                    refresh: true,
                    message: "Not authorized".to_string(),
                })),
        ...
        match block_on(user_exists_result) {
            Ok(user) => {
                ...
                let team_result = async move {
                    ...
                    let query_result = match (&data_access_query.db, &data_access_query.project) {
                        ...
                        (None, Some(_)) => {
                            let json_error = ErrorResponse {
                                status: "fail".to_string(),
                                message: "You must specify a database query with a project query"
                                    .to_string(),
                            };
                    ...
                    match query_result {
                        ...
                        Ok(None) => {
                            let json_error: ErrorResponse = ErrorResponse {
                                status: "fail".to_string(),
                                message: "You do not have access to the requested team data"
                                    .to_string(),
                            };
                        ...
                        Err(_) => {
                            let json_error = ErrorResponse {
                                status: "error".to_string(),
                                message: "Failed to check team permissions for data access"
                                    .to_string(),
                            };
                            Err(ErrorInternalServerError(json_error))
                        }
                ...
                match block_on(team_result) {
                    Ok(team) => ready(Ok(JwtDataMiddleware {
                        access_token_uuid,
                        user,
                        team,
                    })),
        ...
        match block_on(user_exists_result) {
            Ok(user) => {
                ...
                match user.roles.contains(&Role::Admin) {
                    true => ready(Ok(JwtAdminMiddleware {
                        access_token_uuid,
                        user,
                    })),
                    false => {
