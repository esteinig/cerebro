...
pub fn get_cerebro_db_collection<T: Send + Sync>(
    data: &Data<AppState>,
    collection: AdminCollection,
) -> Collection<T> {
    let db = data
        .db
        .database(&data.env.database.names.admin_database_name);
...
pub fn get_teams_db_collection<T: Send + Sync>(
    data: &Data<AppState>,
    team: Team,
    collection: TeamAdminCollection,
) -> Collection<T> {
...
pub fn get_teams_db_stage_collection(
    data: &Data<AppState>,
    team: Team,
    collection: &str,
) -> Collection<StagedSample> {
    let db = data.db.database(&team.admin.database);
    ...
    for d in data {
        csv_writer.serialize(d).map_err(|err| {
            return HttpResponse::InternalServerError()
                .json(SampleSummaryResponse::server_error(err.to_string()));
        })?
    }
