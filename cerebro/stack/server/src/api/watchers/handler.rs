...
use cerebro_model::api::{
    ...
    watchers::{
        model::ProductionWatcher,
        response::{
            DeleteWatcherResponse, ListWatchersResponse, PingWatcherResponse,
            RegisterWatcherResponse,
        },
        schema::RegisterWatcherSchema,
    },
};
...
async fn register_watcher(
    ...
    schema: web::Json<RegisterWatcherSchema>,
    _: web::Query<TeamAccessQuery>,
    auth_guard: jwt::JwtDataMiddleware,
) -> HttpResponse {
    let watcher_collection: Collection<ProductionWatcher> =
        get_teams_db_collection(&data, auth_guard.team, TeamAdminCollection::Watchers);
        ...
        .await
    {
        Ok(None) => {}
        Ok(Some(_)) => {
            return HttpResponse::Conflict().json(RegisterWatcherResponse::conflict(
                &schema.id,
                &schema.name,
                &schema.location,
            ))
        ...
        Err(err) => {
            return HttpResponse::InternalServerError()
                .json(RegisterWatcherResponse::server_error(err.to_string()))
        }
    }
...
async fn list_watchers(
    data: web::Data<AppState>,
    query: web::Query<WatcherListQuery>,
    _: web::Query<TeamAccessQuery>,
    auth_guard: JwtDataMiddleware,
) -> HttpResponse {
    let watcher_collection: Collection<ProductionWatcher> =
        get_teams_db_collection(&data, auth_guard.team, TeamAdminCollection::Watchers);
    ...
    match watcher_collection.aggregate(watcher).await {
        Ok(cursor) => {
            ...
            match watchers.is_empty() {
                false => HttpResponse::Ok().json(ListWatchersResponse::success(watchers)),
                true => HttpResponse::NotFound().json(ListWatchersResponse::not_found()),
            }
        }
        Err(err) => HttpResponse::InternalServerError()
            .json(ListWatchersResponse::server_error(err.to_string())),
    }
}
...
#[delete("/watcher")]
async fn delete_watchers(
    data: web::Data<AppState>,
    query: web::Query<WatchersDeleteQuery>,
    _: web::Query<TeamAccessQuery>,
    auth_guard: jwt::JwtDataMiddleware,
) -> HttpResponse {
    let watcher_collection: Collection<ProductionWatcher> =
    ...
    match watcher_collection.delete_many(delete_query).await {
        Ok(delete_result) => {
            if delete_result.deleted_count > 0 {
                HttpResponse::Ok().json(DeleteWatcherResponse::all_deleted())
            } else {
                HttpResponse::NotFound().json(DeleteWatcherResponse::not_found())
            }
        }
        Err(err) => HttpResponse::InternalServerError()
            .json(DeleteWatcherResponse::server_error(err.to_string())),
    }
}
...
async fn delete_watcher(
        ...
        .await
    {
        Ok(deleted) => match deleted {
            Some(watcher) => HttpResponse::Ok().json(DeleteWatcherResponse::success(watcher)),
...
async fn ping_watcher(
        ...
        .await
    {
        Ok(updated) => match updated {
            Some(_) => HttpResponse::Ok().json(PingWatcherResponse::success(new_ping)),
            None => HttpResponse::NotFound().json(PingWatcherResponse::not_found()),
        },
        Err(err) => HttpResponse::InternalServerError()
            .json(PingWatcherResponse::server_error(err.to_string())),
    }
}
