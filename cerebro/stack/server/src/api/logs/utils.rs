...
pub async fn log_database_change(
    data: &web::Data<AppState>,
    team: Team,
    request_log: RequestLog,
) -> Result<(), HttpResponse> {
    let admin_logs_collection: Collection<RequestLog> =
        get_cerebro_db_collection(data, AdminCollection::Teams);
    let teams_logs_collection: Collection<RequestLog> =
        get_teams_db_collection(data, team, TeamAdminCollection::Logs);
