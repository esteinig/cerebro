...
use mongodb::{bson::{doc, from_document, to_bson, Bson}, error::{ErrorKind, WriteFailure}, options::{FindOneOptions, IndexOptions}, IndexModel, Collection};
    ...
    match result {
        Ok(_) => {
            if let Err(err) = append_audit_event(
                &data, audit_team, AuditAction::Upload,
                Some(schema.id.clone()), schema.run_id.clone(), schema.sample_id.clone(),
                audit_actor,
                format!("uploaded '{}' ({} bytes, tier {:?}, retention {:?})", schema.name, schema.size, schema.tier, schema.retention),
            ).await {
                return HttpResponse::InternalServerError().json(RegisterFileResponse::server_error(format!("audit append failed: {}", err)));
            }
            data.metrics.record(&TelemetryEvent::success(TelemetryOp::Upload));
...
async fn list_files(data: web::Data<AppState>, query: web::Query<FileListQuery>,  _: web::Query<TeamAccessQuery>, auth_guard: jwt::JwtDataMiddleware) -> HttpResponse {
    ...
    let pipeline = get_latest_files_paginated_pipeline(query.run_id.clone(), query.sample_id.clone(), query.watcher_id.clone(), query.page as i64, query.limit as i64);
        ...
        Ok(cursor) => {
            ...
            match files_collection.delete_many(delete_query).await {
                Ok(_) => {
                    ...
                    if let Err(err) = append_audit_event(
                        &data, audit_team, AuditAction::Delete,
                        None, query.run_id.clone(), query.sample_id.clone(),
                        audit_actor,
                        format!("bulk delete (all={:?}, run_id={:?}, sample_id={:?}, {} file(s))", query.all, query.run_id, query.sample_id, count),
                    ).await {
                        return HttpResponse::InternalServerError().json(DeleteFilesResponse::server_error(format!("audit append failed: {}", err)));
                    }
    ...
    match files_collection.find_one(doc! { "id": &id }).await {
        Ok(Some(file)) => {
            if let Some(reason) = protection_reason(&file, Utc::now()) {
                data.metrics.record(&TelemetryEvent::rejected(TelemetryOp::Delete));
        ...
        Err(err) => return HttpResponse::InternalServerError().json(DeleteFileResponse::server_error(err.to_string())),
    }
        ...
        Ok(deleted) => {
            match deleted {
                Some(file) => {
                    if let Err(err) = append_audit_event(
                        &data, audit_team, AuditAction::Delete,
                        Some(file.id.clone()), file.run_id.clone(), file.sample_id.clone(),
                        audit_actor, format!("deleted '{}' (fid {})", file.name, file.fid),
                    ).await {
                        return HttpResponse::InternalServerError().json(DeleteFileResponse::server_error(format!("audit append failed: {}", err)));
                    }
                    data.metrics.record(&TelemetryEvent::success(TelemetryOp::Delete));
...
async fn update_file_lifecycle(
    ...
    if let Some(tier) = &schema.tier { changes.push(format!("tier={:?}", tier)); }
    if schema.clear_retain_until { changes.push("retain_until=cleared".into()); }
    else if let Some(ru) = &schema.retain_until { changes.push(format!("retain_until={}", ru.to_rfc3339())); }
    if let Some(ra) = &schema.reported_at { changes.push(format!("reported_at={}", ra.to_rfc3339())); }
    if let Some(lh) = schema.legal_hold { changes.push(format!("legal_hold={}", lh)); }
    if let Some(ar) = schema.archived { changes.push(format!("archived={}", ar)); }
    if let Some(rs) = &schema.restore_state { changes.push(format!("restore_state={:?}", rs)); }
    ...
    let tel_detail = schema.tier.as_ref().map(|t| format!("{t:?}").to_lowercase());
        ...
        .await
    {
        Ok(update_result) => {
            if update_result.matched_count > 0 {
                if let Err(err) = append_audit_event(
                    &data, audit_team, action,
                    Some(id.clone()), None, None,
                    audit_actor, detail,
                ).await {
                    return HttpResponse::InternalServerError().json(UpdateLifecycleResponse::server_error(format!("audit append failed: {}", err)));
                }
                ...
                match files_collection.find_one(doc! { "id": &id }).await {
                    Ok(Some(_)) => {
                        ...
                        data.metrics.record(&TelemetryEvent::rejected(tel_op.unwrap_or(TelemetryOp::TierMove)));
        ...
        Err(err) => HttpResponse::InternalServerError().json(
            UpdateLifecycleResponse::server_error(err.to_string())
        ),
    }
}
...
async fn get_file(
    ...
    match files_collection.find_one(doc! { "id": &id }).await {
        ...
        Err(err) => HttpResponse::InternalServerError().json(GetFileResponse::server_error(err.to_string())),
    }
}
...
async fn report_out_files(
    ...
    let mut cursor = match files_collection.find(filter).await {
        Ok(cursor) => cursor,
        Err(err) => return HttpResponse::InternalServerError().json(ReportOutResponse::server_error(err.to_string())),
    };
    ...
    loop {
        match cursor.try_next().await {
            Ok(Some(file)) => {
                let transition = policy.report_out_transition(file.retention, reported_at, warm_available);
                let audit_detail = format!("reported out -> tier {:?}, retain_until {:?}", transition.target_tier, transition.retain_until);
                ...
                match transition.retain_until {
                    Some(retain_until) => { set.insert("retain_until", to_bson(&retain_until).unwrap()); }
                    None => { set.insert("retain_until", Bson::Null); }
                }

                if let Err(err) = files_collection.update_one(doc! { "id": &file.id }, doc! { "$set": set }).await {
                    return HttpResponse::InternalServerError().json(ReportOutResponse::server_error(err.to_string()));
                }
                if let Err(err) = append_audit_event(
                    &data, audit_team.clone(), AuditAction::ReportOut,
                    Some(file.id.clone()), file.run_id.clone(), file.sample_id.clone(),
                    audit_actor.clone(), audit_detail,
                ).await {
                    return HttpResponse::InternalServerError().json(ReportOutResponse::server_error(format!("audit append failed: {}", err)));
                }
                data.metrics.record(&TelemetryEvent::with_detail(TelemetryOp::ReportOut, TelemetryOutcome::Success, format!("{:?}", transition.target_tier).to_lowercase()));
                updated += 1;
            }
            Ok(None) => break,
            Err(err) => return HttpResponse::InternalServerError().json(ReportOutResponse::server_error(err.to_string())),
    ...
    HttpResponse::Ok().json(ReportOutResponse::success(updated, reported_at.to_rfc3339()))
}


/// Build an [`AuditActor`] from the authenticated principal.
fn actor_of(auth_guard: &jwt::JwtDataMiddleware) -> AuditActor {
    AuditActor { id: auth_guard.user.id.clone(), email: auth_guard.user.email.clone() }
}
...
fn protection_reason(file: &SeaweedFile, now: DateTime<Utc>) -> Option<String> {
    ...
    if let Some(until) = file.retain_until {
        if until > now {
            return Some(format!("file {} is retained until {}", file.id, until.to_rfc3339()));
        }
    }
    None
}
...
fn audit_fail_open() -> bool {
    matches!(std::env::var("CEREBRO_AUDIT_FAIL_OPEN").ok().as_deref(), Some("true") | Some("1"))
}
...
async fn ensure_audit_index(collection: &Collection<AuditEvent>, team_key: &str) {
    ...
    match collection.create_index(model).await {
        Ok(_) => { audit_index_ensured().lock().unwrap().insert(team_key.to_string()); }
...
async fn append_audit_event(
    ...
    let audit_collection: Collection<AuditEvent> = get_teams_db_collection(data, team, TeamAdminCollection::AuditLogs);
    ...
    loop {
        ...
        let (sequence, prev_hash) = match audit_collection
            .find_one(doc! {})
            .with_options(FindOneOptions::builder().sort(doc! { "sequence": -1 }).build())
            .await
        {
            Ok(Some(tip)) => (tip.sequence + 1, tip.hash),
            Ok(None) => (0, AUDIT_GENESIS_HASH.to_string()),
            Err(err) => {
                data.metrics.record(&TelemetryEvent::failure(TelemetryOp::Audit));
                return audit_failure(AuditError::TipLookup(err.to_string()));
            }
        };

        let mut event = AuditEvent::new(
            sequence, Utc::now(), action.clone(),
            file_id.clone(), run_id.clone(), sample_id.clone(),
            actor.clone(), detail.clone(), prev_hash,
        );
        event.seal();

        match audit_collection.insert_one(event).await {
            Ok(_) => {
                data.metrics.record(&TelemetryEvent::success(TelemetryOp::Audit));
                return Ok(());
            }
...
async fn get_audit(
    ...
    let audit_collection: Collection<AuditEvent> = get_teams_db_collection(&data, auth_guard.team, TeamAdminCollection::AuditLogs);

    let cursor = match audit_collection.find(doc! {}).sort(doc! { "sequence": 1 }).await {
        Ok(cursor) => cursor,
        Err(err) => return HttpResponse::InternalServerError().json(AuditTrailResponse::server_error(err.to_string())),
    };
    ...
    let events: Vec<AuditEvent> = all
        .into_iter()
        .filter(|e| match &query.file_id { Some(fid) => e.file_id.as_deref() == Some(fid.as_str()), None => true })
        .filter(|e| match &query.run_id { Some(rid) => e.run_id.as_deref() == Some(rid.as_str()), None => true })
        .collect();
...
async fn expire_files(
    ...
    if let Some(run_id) = &schema.run_id { filter.insert("run_id", run_id); }
    if let Some(sample_id) = &schema.sample_id { filter.insert("sample_id", sample_id); }

    let mut cursor = match files_collection.find(filter).await {
        Ok(cursor) => cursor,
        Err(err) => return HttpResponse::InternalServerError().json(ExpireResponse::server_error(err.to_string())),
    };
        ...
        match cursor.try_next().await {
            Ok(Some(file)) => {
                if !schema.dry_run {
                    ...
                    if let Err(err) = files_collection.update_one(doc! { "id": &file.id }, doc! { "$set": set }).await {
                        return HttpResponse::InternalServerError().json(ExpireResponse::server_error(err.to_string()));
                    }
                    if let Err(err) = append_audit_event(
                        &data, audit_team.clone(), AuditAction::Expiry,
                        Some(file.id.clone()), file.run_id.clone(), file.sample_id.clone(),
                        audit_actor.clone(), format!("quarantined (retain_until {:?} lapsed)", file.retain_until),
                    ).await {
                        return HttpResponse::InternalServerError().json(ExpireResponse::server_error(format!("audit append failed: {}", err)));
                    }
                    data.metrics.record(&TelemetryEvent::success(TelemetryOp::Expire));
                }
                quarantined += 1;
            }
            Ok(None) => break,
...
async fn purge_files(
    ...
    let mut cursor = match files_collection.find(filter).await {
        Ok(cursor) => cursor,
        Err(err) => return HttpResponse::InternalServerError().json(PurgeResponse::server_error(err.to_string())),
    };
            ...
            Ok(Some(file)) => {
                if !schema.dry_run {
                    if let Err(err) = files_collection.delete_one(doc! { "id": &file.id }).await {
                        return HttpResponse::InternalServerError().json(PurgeResponse::server_error(err.to_string()));
                    }
                    if let Err(err) = append_audit_event(
                        &data, audit_team.clone(), AuditAction::Delete,
                        Some(file.id.clone()), file.run_id.clone(), file.sample_id.clone(),
                        audit_actor.clone(), format!("purged after quarantine grace (fid {})", file.fid),
                    ).await {
                        return HttpResponse::InternalServerError().json(PurgeResponse::server_error(format!("audit append failed: {}", err)));
                    }
                    data.metrics.record(&TelemetryEvent::success(TelemetryOp::Purge));
                }
...
async fn restore_transition(
    ...
    let file = match files_collection.find_one(doc! { "id": &id }).await {
        Ok(Some(file)) => file,
        Ok(None) => return HttpResponse::NotFound().json(RestoreTransitionResponse::not_found(&id)),
        Err(err) => return HttpResponse::InternalServerError().json(RestoreTransitionResponse::server_error(err.to_string())),
    };
    ...
    if !current.can_transition_to(&target) {
        data.metrics.record(&TelemetryEvent::rejected(TelemetryOp::Restore));
        return HttpResponse::Conflict().json(RestoreTransitionResponse::invalid_transition(&current.to_string(), &target.to_string()));
    }
    ...
    match &target {
        RestoreState::Requested => { set.insert("restore_requested_at", to_bson(&now).unwrap()); }
    ...
    match files_collection.update_one(filter, doc! { "$set": set }).await {
        Ok(result) if result.matched_count > 0 => {
            if let Err(err) = append_audit_event(
                &data, audit_team, AuditAction::Restore,
                Some(id.clone()), file.run_id.clone(), file.sample_id.clone(),
        ...
        Ok(_) => {
            data.metrics.record(&TelemetryEvent::rejected(TelemetryOp::Restore));
            HttpResponse::Conflict().json(RestoreTransitionResponse::invalid_transition(&current.to_string(), &target.to_string()))
        }
        Err(err) => HttpResponse::InternalServerError().json(RestoreTransitionResponse::server_error(err.to_string())),
    }
}
