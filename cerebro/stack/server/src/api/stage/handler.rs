...
use cerebro_model::api::{
    stage::{
        model::StagedSample,
        response::{
            DeleteStagedSampleResponse, ListStagedSamplesResponse, RegisterStagedSampleResponse,
        },
    },
    teams::model::TeamAdminCollection,
    towers::model::ProductionTower,
};
...
use crate::api::stage::mongo::{
    create_staged_samples_pipeline, get_latest_staged_samples_pipeline,
};
...
async fn get_tower_from_db(
    data: &web::Data<AppState>,
    id: &str,
    auth_guard: &jwt::JwtDataMiddleware,
) -> Result<ProductionTower, HttpResponse> {
    // Get the registered pipeline to use the pipeline staging area
    let tower_collection: Collection<ProductionTower> =
        get_teams_db_collection(&data, auth_guard.team.clone(), TeamAdminCollection::Towers);

    match tower_collection.find_one(doc! { "id": &id }).await {
        ...
        Err(err) => Err(HttpResponse::InternalServerError()
            .json(RegisterStagedSampleResponse::server_error(err.to_string()))),
    }
}

#[post("/stage/register")]
async fn register_staged_samples(
    data: web::Data<AppState>,
    schema: web::Json<RegisterStagedSampleSchema>,
    ...
    let aggregate_pipeline = create_staged_samples_pipeline(
        &schema.into_inner(),
        &tower,
        &access.team,
        &access.db,
    ...
    let files_collection: Collection<SeaweedFile> =
        get_teams_db_collection(&data, auth_guard.team.clone(), TeamAdminCollection::Files);

    let staged_samples: Vec<StagedSample> =
        match files_collection.aggregate(aggregate_pipeline).await {
            Ok(cursor) => {
                cursor
                    .try_collect::<Vec<_>>()
                    .await
    ...
    let stage_collection: Collection<StagedSample> =
        get_teams_db_stage_collection(&data, auth_guard.team, &tower.stage);

    match stage_collection.insert_many(&staged_samples).await {
        Ok(_) => {
...
#[get("/stage/{tower_id}")]
async fn list_staged_samples(
    data: web::Data<AppState>,
    tower_id: web::Path<String>,
    query: web::Query<StageListQuery>,
    ...
    let aggregate_pipeline =
        get_latest_staged_samples_pipeline(query.run_id.clone(), query.sample_id.clone());

    match stage_collection.aggregate(aggregate_pipeline).await {
        Ok(cursor) => {
        ...
        Err(err) => HttpResponse::InternalServerError()
            .json(ListStagedSamplesResponse::server_error(err.to_string())),
    }
}
...
async fn delete_staged_samples(
    data: web::Data<AppState>,
    tower_id: web::Path<String>,
    query: web::Query<StageDeleteQuery>,
    _: web::Query<TeamAccessQuery>,
    auth_guard: jwt::JwtDataMiddleware,
) -> HttpResponse {
    let tower = match get_tower_from_db(&data, &tower_id, &auth_guard).await {
    ...
    match stage_collection.delete_many(delete_query).await {
        Ok(delete_result) => {
            if delete_result.deleted_count > 0 {
                HttpResponse::Ok().json(DeleteStagedSampleResponse::all_deleted())
            } else {
                HttpResponse::NotFound().json(DeleteStagedSampleResponse::not_found())
            }
        }
        Err(err) => HttpResponse::InternalServerError()
            .json(DeleteStagedSampleResponse::server_error(err.to_string())),
    }
}
