...
pub async fn main() -> std::io::Result<()> {
    ...
    match cli.command {
        Commands::Run(args) => {
            ...
            let mongo_client =
                match MongoClient::with_uri_str(&config.database.connections.mongodb_uri).await {
                    Ok(client) => {
            ...
            let redis_client_auth_session = match RedisClient::open(
                config
                    .database
                    .connections
                    .redis_auth_session_uri
                    .to_owned(),
            ) {
                Ok(client) => {
            ...
            let redis_client_auth_onetime = match RedisClient::open(
                config
                    .database
                    .connections
                    .redis_auth_onetime_uri
                    .to_owned(),
            ) {
                Ok(client) => {
            ...
            if matches!(
                ...
                let faktory_scheduler = Scheduler::new(
                    ...
                    config
                        .database
                        .names
                        .admin_database_locks_collection
                        .clone(),
                );
                ...
                if matches!(
                    std::env::var("CEREBRO_SEED_LIFECYCLE_SCHEDULES")
                        .ok()
                        .as_deref(),
                    Some("true") | Some("1")
                ) {
                ...
                log::info!(
                    "Faktory job scheduler disabled (set CEREBRO_SCHEDULER_ENABLED=true to enable)"
                );
            }
            ...
            match mongo_client.list_database_names().await {
                ...
                Err(err) => {
                    log::error!(
                        "Could not connect to MongoDB main database: {}",
                        err.to_string()
                    );
                    std::process::exit(1);
                }
            ...
            match redis_client_auth_session.get_connection() {
                ...
                Err(err) => {
                    log::error!(
                        "Could not connect to Redis session database: {}",
                        err.to_string()
                    );
                    std::process::exit(1);
                }
            ...
            match redis_client_auth_onetime.get_connection() {
                ...
                Err(err) => {
                    log::error!(
                        "Could not connect to Redis one-time database: {}",
                        err.to_string()
                    );
                    std::process::exit(1);
                }
