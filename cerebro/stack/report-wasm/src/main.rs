...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Compile(args) => {
            let mut compiler = LibraryReportCompiler::new(String::from("/"), args.report)?;
        ...
        Commands::Template(args) => match args.report {
            ReportType::PathogenDetection => match args.template {
            ...
            ReportType::TrainingCompletion => match args.template {
                TemplateFormat::Json => {
                    TrainingCompletionReport::default().to_json(&args.output)?
                }
                TemplateFormat::Toml => {
...
fn main() {
    eprintln!(
        "Command-line interface feature is not enabled. You can enable it with: `--features cli`."
    );
}
