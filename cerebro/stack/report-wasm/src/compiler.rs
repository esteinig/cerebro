...
impl LibraryReportCompiler {
    ...
    pub fn report(&mut self, report: &dyn ReportConfig) -> anyhow::Result<String> {
        self.templates
            .render(&self.report_type, report, self.logo_width.clone())
            .map_err(|e| anyhow::anyhow!("failed to render report template: {}", e.to_string()))
    }
    pub fn report_from_path(
        &mut self,
        config: &Path,
        format: TemplateFormat,
    ) -> anyhow::Result<String> {
        let report: Box<dyn ReportConfig> = match self.report_type {
            ReportType::PathogenDetection => match format {
        ...
        self.templates
            .render(&self.report_type, report.as_ref(), self.logo_width.clone())
            .map_err(|e| anyhow::anyhow!("failed to render report template: {}", e.to_string()))
    }
    pub fn typst(&mut self, text: &str, output: &Path) -> anyhow::Result<()> {
        std::fs::write(output, text)
            .map_err(|e| anyhow::anyhow!("Failed to write Typst file: {:?}", e))
    }
    pub fn svg(&mut self, text: &str, vpath: String, output: &Path) -> anyhow::Result<()> {
        let document = self
            .world
            .compile(text.to_string(), vpath, self.logo_bytes.clone())?;
