...
impl NextflowConfig {
    pub async fn new(
        ...
        if !databases.exists() {
            log::error!(
                "Failed to detect database directory: {}",
                databases.display()
            )
        }
        if !main.exists() {
            log::error!(
                "Failed to detect main script for Nextflow: {}",
                databases.display()
            )
        }
        if !config.exists() {
            log::error!(
                "Failed to detect main configuration for Nextflow: {}",
                databases.display()
            )
        }
        if !workdir.exists() {
    ...
    pub async fn create(&mut self, samples: &Vec<StagedSample>) -> Result<PathBuf, TowerError> {
        ...
        log::info!(
            "Creating execution directory for Nextflow at: {}",
            path.display()
        );
        ...
        log::info!(
            "Writing staged samples to execution directory (n = {})",
            samples.len()
        );
...
impl CerebroTower {
    ...
    pub async fn watch(&self, tower_id: &str, delete_from_stage: bool) -> Result<(), TowerError> {
        ...
        let watchtower_handle = task::spawn(watchtower_task(
            self.client.clone(),
            tower_id.to_string(),
            delete_from_stage,
            tx.clone(),
        ...
        let process_manager_handle = task::spawn(process_manager_task(
            self.nextflow.clone(),
            self.client.clone(),
...
async fn watchtower_task(
    ...
    loop {
        let response = client
            .pull_staged_samples(&tower_id, delete_from_stage)
            .await;
                        ...
                        match child.wait().await {
                            ...
                            Ok(exit_status) => {
                                ...
                                if exit_status.success() {
                                    if let Some(execution_dir) =
                                        pipeline.execution_directory().cloned()
                                    {
                                        match crate::orchestrate::finalize_run(
                                            fs_client.clone(),
                                            execution_dir,
                                            data,
                                        )
                                ...
                                if allow_cleanup {
                                    if let Err(err) = pipeline.cleanup().await {
                                        log::error!(
                                            "Failed to cleanup execution directory: {}",
                                            err.to_string()
                                        )
                                    }
                                } else {
