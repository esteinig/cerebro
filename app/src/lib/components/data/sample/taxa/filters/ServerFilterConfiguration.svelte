...
<div>
    ...
    <p class="pt-4"><span class="opacity-40">Run controls</span></p>
    <div class="p-4">
        <div class="grid grid-rows-2  gap-y-2">
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 gap-y-4 gap-x-4 w-full text-sm">
                <label class="label">
            ...
            <div class="text-sm">
                <span class="text-xs opacity-40">More than {serverFilterConfig.ntc_ratio} x RPM must be present in selected control libraries compared to the sample library to remove tool-specific evidence for a taxon. Control RPM are summed if multiple controls are selected. Ratios are computed separately for DNA and RNA.</span>
            </div>
        </div>
    </div>
    ...
    <div class="p-4">
        <div class="grid grid-rows-2 gap-y-2">
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 gap-y-4 gap-x-4 w-full text-sm">
                <label class="label">
                    <span class="text-xs opacity-60 flex items-center">Prevalence contamination threshold<CircleIndicator circleClass="ml-auto" /><CircleIndicator circleClass="ml-1" color="bg-secondary-500"/><CircleIndicator circleClass="ml-1 mr-2" color="bg-tertiary-500"/></span>
