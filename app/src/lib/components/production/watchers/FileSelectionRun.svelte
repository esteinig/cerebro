...
<div class="grid grid-cols-8 gap-8 items-center">
    ...
    <div class="opacity-60 col-span-1 ml-3">{numberLibraries} {numberLibraries == 1 ? "library" : "libraries"}</div>
    <div class="opacity-60 col-span-1 ml-1">{numberFiles} {numberFiles == 1 ? "file" : "files"}</div>
    <div class="opacity-60 col-span-2"><span class="mr-2">{getDateTimeStringUtc(minDate, true, true)}</span></div>
</div>
