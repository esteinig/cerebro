...
process CalibDeduplication {
    ...
    """
    cerebro workflow tools umi-prepare-calib --input $forward --output ${id}_calib_1.fq.gz 
    calib -f ${id}_calib_1.fq.gz -r $reverse -o ${id}. -c $task.cpus -l1 12 -l2 0 -e 0 -t 2 -k 8 -m 7 --gzip-input 
    cerebro workflow tools umi-dedup-calib --input $forward $reverse --output ${id}_calib_dedup_1.fq.gz ${id}_calib_dedup_2.fq.gz --clusters ${id}.cluster
...
process NaiveDeduplication {
    ...
    """
    cerebro workflow tools umi-dedup-naive --input $forward $reverse --reads $forward --output ${id}_dedup_1.fq.gz ${id}_dedup_2.fq.gz $no_umi_opt
    """
