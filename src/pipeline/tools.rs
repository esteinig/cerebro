use crate::tools::modules::virus::{AnnotationOptions, Annotation};

use super::{taxon::{Taxon, TaxonomyWarning}, record::{BlastLcaRecord, BlastRecord, DiamondRecord, Kraken2UniqRecord, VircovRecord, VircovScanRemapRecord}, error::WorkflowError};
...
fn get_meta_data_from_result_file(path: &PathBuf) -> Result<(String, String, String), WorkflowError> {
    ...
    let name = crate::pipeline::utils::get_file_stem(path)?;
...
impl Vircov {
    pub fn from(id: String, vircov_report: &PathBuf, annotation_options: &AnnotationOptions, meta_data_fields: bool) -> Result<Self, WorkflowError> {

        let (_, _, db) = match meta_data_fields {
            true => get_meta_data_from_result_file(vircov_report)?,
            false => (String::from("module"), String::from("vircov"), String::from("db")),
        };

        let report_file = BufReader::new(File::open(vircov_report)?);
            
        let mut records: Vec<VircovRecord> = vec![];
        for line in report_file.lines() {
            records.push(VircovRecord::from_str(line?, id.clone(), db.clone(), &annotation_options.field_sep, &annotation_options.field_taxid)?)
        }
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VircovScanRemap {
    pub db: String,
    pub taxa: HashMap<String, Taxon>,
    pub records: Vec<VircovScanRemapRecord>,
    pub taxonomy_warnings: Vec<TaxonomyWarning> 
}

impl VircovScanRemap {
    pub fn from(id: String, vircov_report: &PathBuf, annotation_options: &AnnotationOptions, meta_data_fields: bool) -> Result<Self, WorkflowError> {
        ...
        let mut records: Vec<VircovScanRemapRecord> = vec![];
        for (i, line) in report_file.lines().enumerate() {
            if i == 0 {
                continue; // Skip for now, will implement normal CSV reader
            }
            records.push(VircovScanRemapRecord::from_str(&line?, &annotation_options)?)
        }
        
        Ok(Self { db, taxa: HashMap::new(), records, taxonomy_warnings: Vec::new() })
    }
    pub fn from_vircov(id: String, vircov: Vircov, annotation_options: &AnnotationOptions) -> Result<Self, WorkflowError> {
                
        Ok(Self { db: vircov.db.to_string(), taxa: HashMap::new(), records: vircov.records.into_iter().map(|r| {

            let scan_annotation = Annotation::from(&r.description, &annotation_options);
            VircovScanRemapRecord::from(id.clone(), vircov.db.clone(), r, scan_annotation, None, None, None)

        }).collect(), taxonomy_warnings: vircov.taxonomy_warnings })
    }
    pub fn get_taxa(&mut self, taxonomy: &GeneralTaxonomy) -> Result<(), WorkflowError>{
        if !self.taxa.is_empty() {
            return Err(WorkflowError::TaxAggregate("Vircov".to_string()))
        }
        for record in &self.records {
            // Get the taxon for this record from its associated taxid
            let taxid = match &record.taxid {
                Some(taxid) => taxid,
                None => return Err(WorkflowError::VircovScanRemapTaxidMissing)
            };
            let mut _taxon_result = Taxon::from_taxid(taxid.clone(), taxonomy, true);


            // At the moment we remove non-matching taxonomy queries from result - we need to completely synchronize databases with current taxonomy.
            if let Err(_) = _taxon_result {
                log::warn!("Could not find taxid `{}` in taxonomy! See the warning table output. Please ensure that databases and taxonomy are synchronized.", &taxid);
                self.taxonomy_warnings.push(TaxonomyWarning::new(&record.id, "Vircov", &record.db, "Vircov::get_taxa", format!("{} {} {}", &taxid, "", record.reference_description)));
                continue;
            }
            ...
            if let Some(taxon) = self.taxa.get_mut(&_taxon.taxid) {
                ...
                // ... otherwise add the evidence to the just created taxon and insert it into colletion
                _taxon.evidence.alignment.push(record.clone());
                self.taxa.insert(taxid.to_string(), _taxon);
            }
        }
        Ok(())
    }
