...
use crate::pipeline::record::VircovRecord;
use crate::pipeline::record::VircovScanRemapRecord;
use crate::tools::utils::get_seq_record_identifier;
use crate::pipeline::tools::Vircov;
...
impl VirusAlignmentSummary {
    pub fn new(
        ...
        let scan_vircov = Vircov::from(
            ...
            &annotation_options,
            false
        ).map_err(|_| ToolError::VircovScanParseFailed)?;

        let remap_vircov = Vircov::from(
            id.to_string(), &remap, &annotation_options, false
        ).map_err(|_| ToolError::VircovRemapParseFailed)?;
        ...
        for scan_record in scan_vircov.records {
            ...
            let matched_records = match matching_records.len() {
                ...
                _ => {
                    ...
                    return Err(ToolError::VircovRemapMatchNotIdentifiable(scan_record.reference.clone()))
                }
            };
                ...
                Some(remap_record) => {
                    ...
                    match matching_records.len() {
                        ...
                        _ => {
                            ...
                            return Err(ToolError::VircovConsensusSequenceMatchNotIdentifiable(remap_record.reference.clone()))
                        }
                    }
                }
            };
                ...
                Some(remap_record) => {
                    ...
                    match matching_records.len() {
                        ...
                        _ => {
                            ...
                            return Err(ToolError::VircovRemapMatchNotIdentifiable(remap_record.reference.clone()))
                        }
                    }
                }
            };
    ...
    pub fn parse_coverage_data(data: &PathBuf) -> Result<Vec<CoverageRecord>, ToolError> {
        ...
        for rec in reader.deserialize() {
            coverage_records.push(rec.map_err(|err| ToolError::CsvRecordNotParsed(err))?)
        }
        Ok(coverage_records)
    }
    pub fn parse_consensus_sequences(fasta: &PathBuf) -> Result<Vec<ConsensusRecord>, ToolError>{
