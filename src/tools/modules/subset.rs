...
use crate::tools::utils::get_compression_writer;
...
impl FastaSubset {
    pub fn from_mash(path: &Path, min_identity: &f64, min_shared_hashes: &u32) -> Result<Self, ToolError> {
    ...
    pub fn select_grouped(&self, group_index: &Option<usize>, group_by: &Option<String>, group_sep: &String) -> Result<HashSet<String>, ToolError> {
                    ...
                    Some(group_idx) => {
                        ...
                        for record in mash_data {
                            ...
                            match group_id {
                                ...
                                None => return Err(ToolError::GroupIndexError(group_idx.to_string(), record.query_id.clone()))
                            }
                        }
                        Some(grouped)
                    },
                                ...
                                for record in mash_data {
                                    ...
                                    let group_id: Vec<Option<&str>> = record.query_comment.split(group_sep).map(
                                        ...
                                        if group_id.is_empty() {
                                            return Err(ToolError::GroupIndexError("no id field".to_string(), record.query_id.clone()))
                                        }
                                    ...
                                    match group_id[0] {
                                        ...
                                        None => return Err(ToolError::GroupIndexError("no id field value".to_string(), record.query_id.clone()))
                                    }
                                }
                                Some(grouped)
                            },
                ...
                Some(grouped_data) => {
                    ...
                    for (group, records) in grouped_data {
                        ...
                        match record {
                            ...
                            None => return Err(ToolError::GroupSelectionError(group))
                        }
                    }
                    Ok(seq_ids)
                },
    ...
    pub fn get_record_ids(&self, group_index: &Option<usize>, group_by: &Option<String>, group_sep: &String) -> Result<HashSet<String>, ToolError> {
        Ok(self.select_grouped(group_index, group_by, group_sep)?)
    }
    pub fn subset(&self, seq_ids: HashSet<String>, fasta: &PathBuf, output: &PathBuf, output_format: &Option<niffler::compression::Format>, compression_level: &niffler::compression::Level) -> Result<(), ToolError> {
...
impl MashScreenRecord {
    pub fn from_str(line: String) -> Result<Self, ToolError>{
