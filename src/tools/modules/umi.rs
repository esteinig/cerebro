...
impl Umi {
    ...
    pub fn trim_umi_index(&self, umi_fastq: &PathBuf, output: &PathBuf, head_trim: usize) -> Result<(), ToolError> {
        ...
        while let Some(record) = reader.next() {
            ...
            let qual = match rec.qual() { 
                ...
                None => return Err(ToolError::QualityScoresNotPresent)
            };
    ...
    pub fn prepare_calib(&self, fastq: &PathBuf, output: &PathBuf) -> Result<(), ToolError> {       
        ...
        while let Some(record) = reader.next() {
            ...
            let mut umi_seq = match rec_id.split(":").collect::<Vec<&str>>().split_last() {
                ...
                None => return Err(ToolError::UmiFieldNotFound(rec_id))
            };
    ...
    pub fn naive_dedup(&self, input: &Vec<PathBuf>, output: &Vec<PathBuf>, forward: &PathBuf, summary: &Option<PathBuf>, prepend_umi: bool) -> Result<Vec<()>, ToolError> {

        if !input.len() == output.len() {
            return Err(ToolError::InputOutputFileMismatch)
        }
        ...
        for reads in clusters.values() {
            ...
            let best_read_mean_quality_forward = match qualities.first() {
                ...
                None => return Err(ToolError::CalibClusterPickNotAvailable)
            };
        ...
        let result: Result<Vec<()>, ToolError> = input.par_iter().enumerate().map(|(idx, fq)| -> Result<(), ToolError> {
    ...
    pub fn calib_dedup(&self, fastq: &Vec<PathBuf>, output: &Vec<PathBuf>, calib_clusters: &PathBuf, summary: Option<PathBuf>, identifiers: Option<PathBuf>) -> Result<Vec<()>, ToolError> {
        ...
        for record in reader.deserialize() {
            let rec: CalibClusterReportRecord = record.map_err(|err| ToolError::CalibClusterRecordNotValid(err))?;
        ...
        let result: Result<Vec<()>, ToolError> = fastq.par_iter().enumerate().map(|(idx, fq)| -> Result<(), ToolError> {
            let mut reader = parse_fastx_file(fq)?;
            let mut writer = get_compression_writer(&output[idx], &self.output_format, &self.compression_level)?;
    ...
    pub fn check_umi_in_read(&self, fastq: &Vec<PathBuf>, output: &PathBuf) -> Result<(), ToolError> {

        let records: Result<Vec<CheckRecord>, ToolError> = fastq.par_iter().map(|fq| -> Result<CheckRecord, ToolError> {

            let name = get_file_stem(fq).map_err(|err| ToolError::FileStemNotExtracted(err))?;
