...
use actix_cors::Cors;
use clap::Parser;
use crate::terminal::{App as Cli, Commands, StackCommands};
...
pub struct AppState {
    ...
    pub env: crate::api::config::Config,
}
        ...
        Commands::Stack ( subcommand ) => {
            ...
            match subcommand {
                StackCommands::RunServer( args ) => {
                    ...
                    let config = match crate::api::config::Config::from_toml(&args.config) {
                        Ok(config) => {
