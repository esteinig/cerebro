use crate::api::{auth::{jwt, token, schema::{AuthLoginSchema, AuthResetPasswordSchema, AuthOneTimeCheckSchema, TokenDetails}}, config::Config, logs::model::Action, utils::get_cerebro_db_collection};
use std::{fmt, str::FromStr};
use crate::api::email::Email;
use crate::api::users::model::{User, UserId, Role};
    ...
    match token_type {
        OneTimeTokenType::UserVerificationCheck => {
            ...
            match create_one_time_token(
                ...
                &data.env.security.token.expiration.onetime_max_age_reset  // short because immediate redirect
            ).await
            {   
                Ok(one_time_token_details) => {
                    HttpResponse::Ok()
